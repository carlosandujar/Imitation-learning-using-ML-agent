...
public class PadelAgentX : Agent
{
    ...
    public void MoveAgent(ActionSegment<int> discreteActions, ActionSegment<float> ContinuousActions)
    {
        ...
        if (hitBall)
        {
            ...
            {
                const float MAXFRAMES = 2000.0f;
                float length = environmentController.agentsController.padelAgentsList[0].StepCount;
                float reward = EnvironmentControllerX.HittingBallReward;
                if (true)
                {
                    reward = EnvironmentControllerX.HittingBallReward * (1f - length / MAXFRAMES);
                }
                
                environmentController.AddTeamRewards(team, reward);
