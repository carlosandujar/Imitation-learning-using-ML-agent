...
public class KeyPositionsController : MonoBehaviour
{
    ...
    public void CalculateKeyPositionsRelatedRewards(PadelAgentX player)
    {
        ...
        if (true) // CAG
        {
            if (player.GetRole()!=Role.Receiver) return;  //  position rewards just for receiver
        }

        const float MAXFRAMES = 1000.0f;
        float length = environmentController.agentsController.padelAgentsList[0].StepCount;

        if (Vector3.Distance(targetPosition, currentPosition) < 1.5f)
        {
            float reward = EnvironmentControllerX.StayingAroundKeyPositionsReward;
            if (true)
            {
                reward = EnvironmentControllerX.StayingAroundKeyPositionsReward * (1 - (length / MAXFRAMES));
            }
        ...
        {
            float reward = EnvironmentControllerX.ApproachingKeyPositionsReward;
            if (true)
            {
                reward = EnvironmentControllerX.ApproachingKeyPositionsReward * (1 - (length / MAXFRAMES));
            }
            player.AddReward(reward);
